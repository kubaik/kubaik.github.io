...
The confusion starts with a category error. Engineers treat an LLM agent like a microservice, because the interface looks similar: you send JSON, you get JSON back. But a microservice is deterministic given its inputs. An agent is a stochastic policy over a tool space, and the tool space is defined by whatever you expose.
...
Think of it like a database driver exposed to a SQL generator you didn't write. You wouldn't hand a generated query string to `psycopg` and hope. You'd parameterize, you'd set a statement timeout, you'd run it against a read replica first. The agent is the SQL generator; your internal tool is the driver.

That analogy gives you four concrete design rules:

1. **Parameterize everything.** Every agent-facing tool takes typed, validated arguments. No free-form `payload: object`. If you need flexibility, use a discriminated union (Pydantic v2 `Field(discriminator=...)` or a TypeScript discriminated union) so the model has to pick from a closed set.
...
class RefundRequest(BaseModel):
    order_id: str = Field(pattern=r"^ord_[A-Za-z0-9]{16}$")
    amount: Decimal = Field(gt=0, le=Decimal("10000"))
    currency: Literal["USD", "EUR", "SGD"]
    region: Region
...
The `dry_run: bool = True` default is the important line. It means a model that forgets to set it still can't issue a real refund. The `Region` enum means the model cannot invent a region that doesn't exist. The regex on `order_id` catches the common hallucination where the model produces `ord_123` because it pattern-matched from the prompt instead of from a real lookup.
...
```javascript
// Node 20 LTS, using zod 3.23 for runtime validation
import { z } from "zod";

const DeployArgs = z.object({
  service: z.enum(["api", "worker", "web"]),
  version: z.string().regex(/^v\d+\.\d+\.\d+-[0-9a-f]{7}$/), // git SHA required
  environment: z.enum(["staging", "prod"]),
  confirmation: z.literal("CONFIRM_DEPLOY"), // model must echo this
  idempotency_key: z.string().uuid(),
});

export async function deploy(rawArgs) {
  const args = DeployArgs.parse(rawArgs); // throws on any mismatch
  if (args.environment === "prod") {
    await requireApproval(args); // human-in-the-loop via Slack
  }
...
| Concern | Guardrail | Where it lives |
|---|---|---|
| Invalid arguments | Typed schema (Pydantic 2.7 / zod 3.23) | Tool definition |
| Hallucinated values | Enums, regex patterns, discriminated unions | Tool definition |
| Accidental writes | `dry_run: bool = True` default | Tool definition |
| Irreversible actions | Human approval, confirmation literal | Broker |
| Retry storms | Idempotency key, circuit breaker | Broker |
| Runaway loops | Wall-clock budget (5s read / 30s write) | Broker |
...
**How do I stop an agent from calling a destructive tool by accident?**
Make the destructive path require an explicit, hard-to-hallucinate confirmation — a literal string like `"CONFIRM_DEPLOY"` that the model has to echo, plus a human approval step for anything irreversible. Default every mutating tool to `dry_run: True` so the model has to opt in to real execution. This doesn't eliminate the risk, but it moves the failure from "accidental prod write" to "agent asks for approval," which is a recoverable state.

**Why does my agent keep retrying the same failing tool call?**
Because the error message doesn't tell it what to do differently. A generic `400 Bad Request` gives the model nothing to work with, so it resamples the same distribution and produces the same invalid argument. Return structured errors with the allowed values, the field that failed, and a hint. Also add a per-tool retry cap (3 is typical) and a circuit breaker — after N failures in a window, stop calling and return a terminal error the agent must surface to the user.

**What's the difference between structured output and a validated tool schema?**
Structured output guarantees the JSON shape matches a schema you provided. A validated tool schema additionally checks semantic constraints — enums, regex patterns, numeric ranges, cross-field rules — at the point the tool executes. You want both. Structured output alone will happily produce `region: "us-east-1"` for a Frankfurt customer because the shape is valid; only a semantic validator catches that.

**Do I need a vector database to build internal agent tooling?**
Almost certainly not. The storage you need is a trace table — prompt, tool calls, arguments, results, outcome — and Postgres with a JSONB column handles millions of rows fine. Vector databases solve similarity search over embeddings, which is a different problem. If you're reaching for one before you have trace replay working, you're solving the wrong problem first.
