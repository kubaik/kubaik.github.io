...
# FIX (found in review, 2026): this file previously disallowed /tag/
# entirely, which blocked Googlebot from crawling EVERY tag page —
# including the ones _generate_tag_pages() explicitly marks
# <meta name="robots" content="index, follow"> because they clear the
# thin-content threshold (>= 8 posts). A per-page meta robots directive
# is irrelevant if the crawler is never allowed to fetch the page in the
# first place, so the two mechanisms were contradicting each other and
# the "indexed" tag pages were in practice unreachable. Thin tag pages
# (< 8 posts) still self-declare noindex via their own meta tag and
# don't need a robots.txt block on top of that.
...
"""
    ...
    def _generate_post_pages(self, posts: List[BlogPost]):
        ...
        for i, post in enumerate(posts):
            ...
            # sharing for every post.  We check whether an OG card generated by
            # generate_og_card() actually exists on disk before setting the flag.
            #
            # FOLLOW-UP FIX (found in review, 2026): BUG-7's check only ever
            # looked for a `.png`, but adsense_fixes/image_optimizer.py's
            # generate_og_card() — the OG generator actually called from the
            # live `auto` pipeline — writes `.svg`, not `.png`. That mismatch
            # meant has_og_image was False for every post produced by `auto`
            # (it only became True later, and only for posts re-rendered
            # after someone separately ran a CLI mode that triggers the
            # Pillow-based PNG generator). Checking both extensions here so
            # whichever generator actually ran is picked up; og_img_ext below
            # carries which one was found so the template can reference the
            # right file instead of assuming .png.
...
def _build_templates() -> dict:
    ...
    <head>
        ...
        {%- set og_img_actual = base_url + '/static/og/' + post.slug + '.' + (post.og_image_ext or 'png') %}
        {%- set og_img_fallback = base_url + '/static/icons/icon-512x512.png' %}
        {#- NOTE (fixed in review, 2026): previously hardcoded '.png' here,
            which silently never matched the .svg files the live OG
            generator actually writes. og_image_ext is now set in
            _generate_post_pages() based on whichever file exists on disk.
            SVG is still a weaker choice than PNG/JPEG for og:image on
            several platforms' crawlers (X/Twitter, LinkedIn, Facebook) —
            treat has_og_image=True + ext=svg as "works, not ideal", and
            prefer producing a real .png where possible. #}
        <meta property="og:image" content="{{ og_img_actual if post.has_og_image else og_img_fallback }}">
...
<head>
    ...
    <meta name="description" content="Kubai Kevin is a software developer based in Nairobi, Kenya with a background in Python, Node.js, and AWS. This site publishes AI-assisted, human-curated writing on AI, backend systems, and developer careers.">
    <meta name="base-path" content="{{ base_path }}">
    <link rel="canonical" href="{{ base_url }}/about/">
    <meta property="og:type" content="profile">
    <meta property="og:title" content="About Kubai Kevin — {{ site_name }}">
    <meta property="og:description" content="Software developer in Nairobi curating AI-assisted writing on AI, backend systems, and developer careers.">
    ...
    <script type="application/ld+json">
    {
      ...
      "mainEntity": {
        ...
        "familyName": "Kubai",
        "jobTitle": "Software Developer",
        "description": "Software developer based in Nairobi, Kenya with a background in Python, Node.js, and AWS, focused on fintech backends and AI integration. Curates and edits an AI-assisted publication covering backend systems, AI, and developer careers.",
        "url": "{{ base_url }}/about/",
...
<body>
    ...
    <main class="container">
        <div class="about-hero">
            ...
            <p>AI-assisted, human-curated writing about software development, AI, and developer careers.</p>
        </div>
        ...
        <article itemscope itemtype="https://schema.org/Person"
            ...
            <div class="author-profile">
                ...
                <div>
                    ...
                    <p class="author-bio">
                        I started this site to make the practical, real-world side of backend
                        and AI engineering easier to find. Articles are drafted with AI assistance
                        and grounded in official documentation, established best practices, and
                        patterns I've encountered in my own work — see the
                        <a href="#editorial">editorial process</a> below for exactly what that
            ...
            <div class="section-card">
                <h2>Technologies covered on this site</h2>
                <p style="color:#555;font-size:0.9rem;margin-bottom:0.5rem">
                    Tools and systems this site writes about most often, reflecting my own
            ...
            <div class="section-card" id="editorial">
                ...
                <div class="process-step">
                    ...
                    <div class="step-body">
                        ...
                        <p>Topics are drawn from a curated list of recurring themes in backend
                        engineering, AI systems, and developer careers — areas I have direct
                        interest and background in, covering patterns and trade-offs that come
                        up often in real production work.</p>
                    </div>
                </div>
                ...
                <div class="process-step">
                    ...
                    <div class="step-body">
                        ...
                        documentation and established best practices. Some articles use a
                        first-person "I" or "we" narrative with specific numbers or incidents
                        to make trade-offs concrete — treat these as illustrative scenarios
                        composed to reflect common real-world patterns, not as verified case
                        studies from a specific company or system. Given the volume published
                        here, not every article gets individual line-by-line review before it
                        goes live — I rely on the correction process below to catch and fix
                        mistakes.</p>
                    </div>
                </div>
